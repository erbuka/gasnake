class MainMenuScene extends Scene {
    ...
    renderTitle(time, data) {
        ...
        ctx.save();
        {
            ctx.translate((this.app.viewport.width - data.spriteWidth) / 2, this.app.viewport.height / 4);

            // Draw shadows
            ctx.fillStyle = Constants.Colors.Shadow0;
            for (let i = 0; i < idx; i++)
                ctx.fillRect(data.cells[i].x + 0.1, data.cells[i].y + 0.1, 1, 1);

            ctx.save();
            {
                ctx.globalAlpha = idxDelta;
                ctx.fillRect(data.cells[idx].x + 0.1, data.cells[idx].y + 0.1 + (1 - idxDelta), 1, 1);
            }
            ctx.restore();
